...
func ServiceFile(name, singularName string, auth, documentation, logger bool) [2]string {
	...
	imports := fmt.Sprintf("import { Create%sDto } from './dtos/create-%s.dto';\nimport { Update%sDto } from './dtos/update-%s.dto';\nimport { %s } from './entities/%s.entity'\nimport { Repository } from 'typeorm';\nimport { InjectRepository } from '@nestjs/typeorm';\n", titledSingularName, singularName, titledSingularName, singularName, titledSingularName, singularName)
	headerDecorators := "@Injectable()\n"
	constructorContent := fmt.Sprintf("@InjectRepository(%s) private readonly %sRepository: Repository<%s>,", titledSingularName, singularName, titledSingularName)

	methods := [6]string{}

	// * Method to create
	methods[0] = fmt.Sprintf("\tasync create(create%sDto: Create%sDto){\n\t\t\tconst %s = this.%sRepository.create(create%sDto);\n\t\t\treturn await this.%sRepository.save(%s)\n\t}", titledSingularName, titledSingularName, singularName, singularName, titledSingularName, singularName, singularName)
